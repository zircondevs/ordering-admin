...
const BreakdownSummary = ({  order}:any) => {
	...
	return (
		<div>
			<Total height="auto">
				<Container >
					<Flex justifyContent="space-between" wrap="nowrap" alignItems="flex-start">
						<Span  fontFamily='light' weight="700" lineHeight="16" size="14" colour={"Grey.3"}>
							Order ID
						</Span>
						<Time  fontFamily='light' weight="400" lineHeight="19" size="14" colour={"Grey.1"} textAlign="end">
					...
					<Spacer height="8px"/>
					<OrderId  fontFamily='regular' weight="700" lineHeight="28" size="24" colour={"Grey.1"}>
						{(order?.orderId)}
					</OrderId>
				</Container>
							...
							{order?.item?.map((item: any, idx: number) => (
								<Flex key={idx} justifyContent="space-between" height="auto"  width="auto">
									<Flex justifyContent="space-between" height="auto">
										<Bold  fontFamily='regular' weight="400" lineHeight="16" size="14" colour={"Grey.4"}>
											Pack {idx + 1} {" "}
										</Bold>
										<Bold  fontFamily='regular' weight="400" lineHeight="16" size="12" colour={"Grey.4"}>
											Quantity
										</Bold>
									</Flex>
									<Spacer height="10px"/>
										...
										{item?.length > 0? 
											item?.map((product: any, idx: number) => (
												<Flex key={idx} justifyContent="space-between" height="auto"  width="auto">
													...
													<Span  fontFamily='regular' weight="400" lineHeight="21" size="16" colour={"Grey.2"}>
														{product?.quantity}   
													</Span>
												</Flex>
											))
			...
			<Total height="auto">
				...
				{/* <Grid gap="10px">
					{order?.packs?.map((item: any, idx: number) => (
						<Flex key={idx} justifyContent="space-between" height="auto"  width="auto">
							<div>
								<Bold  fontFamily='regular' weight="700" lineHeight="16" size="12" colour={"Black.default"}>
									Pack {idx + 1} {" "}
									<Span  fontFamily='regular' weight="400" lineHeight="14" size="12" colour={"Grey.3"}>
										({item?.length} items)
									</Span>
								</Bold>
							
							</div>
							<Bold  fontFamily='regular' weight="700" lineHeight="16" size="12" colour={"Grey.2"}>
								{naira} 
				...
				<Grid gap="16px">
					{Object.values(total).map((item, idx) => (
						<Flex key={idx} justifyContent="space-between" height="auto" >
							<div>
								<Bold  fontFamily='regular' weight="700" lineHeight="16" size="14" colour={"Black.default"}>
			...
			<Flex  justifyContent="flex-end" height="auto"  margin="0 0 10px">
				<Container margin="0 16px 0 0" width="auto">
					<Bold  fontFamily='regular' weight="700" lineHeight="16" size="16" colour={"Black.default"}>
						Total:
					</Bold>
				</Container>
				<Bold  fontFamily='bold' weight="700" lineHeight="16" size="18" colour={"Black.default"}>
			...
			
			{
				new Date(order?.updatedAt) === new Date(order?.createdAt) ? null :
					<Span  fontFamily='light' weight="700" lineHeight="16" size="14" colour={"Black.60"}>
						Order updated on 	{" "}
